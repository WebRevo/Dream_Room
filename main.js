...
controls.enableDamping = false; // this is used to control the movements in the screen
controls.enablePan = false;
controls.minDistance = 1;//to fix the minimum distance of the camera view
controls.maxDistance = 3;//to fix the maximum distance of the camera view
  ...
  function (room) {
    ...
    room.scene.children.forEach((child) => {
      ...
      if (child.name === 'CPU') {
        ...
        child.children[2].material.color.set(0x999999);
        child.children[2].material.ior = 3;
        child.children[2].material.transmission = 1;
        child.children[2].material.opacity = 0.8;
      ...
      if (child.name === 'Book') {
        ...
        const bookTexture = new THREE.TextureLoader().load(
          '/FINAL_SARATHI.png' // here we need to add our resume page
        );
...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('SARATHI S', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
